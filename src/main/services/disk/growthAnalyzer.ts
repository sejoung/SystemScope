...
import { saveSnapshot, loadSnapshots, type Snapshot, type FolderSnapshot } from '@main/services/history/snapshotStore'
...
export async function analyzeGrowth(period: string = '7d'): Promise<GrowthViewResult> {
  ...
  const baseline = findBaselineSnapshot(allSnapshots, since)

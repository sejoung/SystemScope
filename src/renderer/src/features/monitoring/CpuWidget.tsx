      ...
      >
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginTop: '2px' }}>
            <span style={{ color: 'var(--text-muted)' }}>{tk('monitoring.cpu.avg_core')}</span>
            <span style={{ color: 'var(--accent-green)', fontWeight: 500 }}>{`${avgCore.toFixed(0)}%`}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span style={{ color: 'var(--text-muted)' }}>{tk('monitoring.cpu.peak_core')}</span>
            <span style={{ color: peakCore > 80 ? 'var(--accent-red)' : 'var(--accent-yellow)', fontWeight: 500 }}>{`${peakCore.toFixed(0)}%`}</span>
          </div>

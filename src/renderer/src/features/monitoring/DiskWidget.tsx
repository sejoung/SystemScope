      ...
      <div>
        {totalIops === null && busyPercent === null ? (
          ...
          <>
            ...
            >
              <InfoRow label={tk('monitoring.disk.read_iops')} value={formatIops(io.readsPerSecond)} color="var(--accent-blue)" />
              <InfoRow label={tk('monitoring.disk.write_iops')} value={formatIops(io.writesPerSecond)} color="var(--accent-purple)" />
              <InfoRow label={tk('monitoring.disk.total_iops')} value={formatIops(totalIops)} color="var(--accent-cyan)" />
              {busyPercent !== null && (
                <InfoRow label={tk('monitoring.disk.busy')} value={`${busyPercent.toFixed(1)}%`} />
              )}
            </div>
          </>
        )}

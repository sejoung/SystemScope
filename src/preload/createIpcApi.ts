...
import type { AppUninstallRequest, CleanupRuleConfig, TrashItemsRequest, TimelineRange, EventQueryOptions, ReportOptions, SaveReportOptions, WorkspaceProfile } from "@shared/types";
...
export function createIpcApi(): SystemScopeApi {
  return {
    ...
    scanStartupItemsWithSystemSettings: () =>
      invokeWithRequestId(IPC_CHANNELS.STARTUP_SCAN_BTM),
    toggleStartupItem: (id: string, enabled: boolean) =>
      invokeWithRequestId(IPC_CHANNELS.STARTUP_TOGGLE, id, enabled),
    openLoginItemsSettings: () =>
      invokeWithRequestId(IPC_CHANNELS.STARTUP_OPEN_SETTINGS),
    ...
    getDevToolsOverview: (options?: { forceRefresh?: boolean }) =>
      invokeWithRequestId(IPC_CHANNELS.TOOLS_GET_OVERVIEW, options ?? {}),
    getProfiles: () => invokeWithRequestId(IPC_CHANNELS.PROFILE_GET_ALL),
    saveProfile: (profile: WorkspaceProfile) =>
      invokeWithRequestId(IPC_CHANNELS.PROFILE_SAVE, profile),

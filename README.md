...
- 아코디언: Quick Scan, Large Files, Extensions, Recently Grown, Duplicate Files 등은 접기/펼치기 지원
  - 접힌 상태에서도 헤더의 액션 버튼으로 바로 실행 가능
  - 실행 완료 시 자동으로 열리며 뱃지로 요약 표시
- 사용자 공간 분석 결과는 Zustand 스토어에 캐싱하여 페이지 이동 시 재호출 방지
- 사용자가 Rescan 버튼으로 원할 때만 갱신

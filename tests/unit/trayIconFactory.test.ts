...
describe('trayIconFactory', () => {
  ...
  it('should render monochrome macOS tray svg with pulse indicator', () => {
    const svg = renderTraySvg({ level: 4, pulse: true, platform: 'darwin' })
    expect(svg).toContain('circle cx="19.4" cy="4.8" r="1.6"')
    expect(svg).toContain('fill="#000000"')
  })

  it('should render colored Windows tray svg', () => {
    const svg = renderTraySvg({ level: 3, pulse: false, platform: 'win32' })

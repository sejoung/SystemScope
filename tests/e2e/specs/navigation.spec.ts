...
test.describe('사이드바 네비게이션', () => {
  ...
  test('Overview 페이지가 표시된다', async ({ mainWindow }) => {
    await expect(
      mainWindow.locator('[data-testid="page-dashboard"], [data-testid="page-loading"]')
  ...
  test('Applications 페이지로 이동한다', async ({ mainWindow }) => {
    ...
    await expect(mainWindow.getByTestId('page-apps')).toBeVisible({ timeout: 5_000 })
  })

  test('Preferences 페이지로 이동한다', async ({ mainWindow }) => {
    await clickNav(mainWindow, 'settings')
    await expect(mainWindow.getByTestId('page-settings')).toBeVisible({ timeout: 5_000 })
  })
})

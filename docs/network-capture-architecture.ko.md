...
| 실제 mac helper 연결 | 1차 구현 | `mac/NetworkCaptureHost/` XcodeGen 프로젝트 + Swift host/extension, Electron 측 소켓 클라이언트 연결 (mock fallback 유지). 빌드/승인/notarization은 미검증 |
| `networkCapture.mac.ts` 분리 | 완료 | orchestrator + platform collector로 분리, mock은 mac collector로 이동 |
| helper-메인 IPC | 문서화 | `mac/NetworkCaptureHost/README.md`에 Unix socket JSON event stream 계약 정의, 실제 소켓 구현은 미착수 |

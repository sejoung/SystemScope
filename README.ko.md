...
- **타임라인**: 시스템 상태 히스토리, 이벤트 타임라인, 지점 상세, 알림 인텔리전스(지속 알림 및 반복 패턴)
- **알림**: 디스크 / 메모리 / GPU 사용률 알림과 임계치 설정
- **디스크 분석**: 폴더 스캔, 트리맵, 대용량 파일, 확장자 분포, 오래된 파일 탐색, 최근 급성장 폴더, 중복 파일 찾기
- **정리 & 자동화**: 캐시 / 로그 / 빌드 / 임시 경로에 대한 규칙 기반 정리, 검토 인박스, 예약 자동 정리
- **Docker 관리**: 컨테이너, 이미지, 볼륨, 빌드 캐시 조회 및 정리
- **프로세스 관리**: 프로세스 검색, 조회, 종료 — 강제 종료 및 프로세스 트리 전체 종료 포함
- **포트 도구**: 활성 포트 조회와 특정 포트/IP 모니터링
- **개발 도구**: 개발 환경 헬스 체크(Node, Python, Java, Docker, CUDA, PyTorch), 툴체인 캐시 정리, 포트 충돌 센터, 프로젝트 모니터, 워크스페이스 프로필
- **앱 정리**: 설치 앱 제거, 관련 데이터 정리, 잔여 데이터 정리, Windows 잔여 제거 레지스트리 정리
- **성장 추세**: 스냅샷 기반 폴더 성장 분석
- **업데이트 확인**: 패키징된 앱에서 새 릴리즈 확인 및 다운로드 페이지 열기
- **다국어 UI**: 한국어 / 영어
- **트레이 상주 UX**
- **다크 / 라이트 테마**
...
| **Timeline** | 시스템 상태 히스토리, 이벤트 타임라인, 지점 상세, 알림 인텔리전스 |
| **Storage** | 폴더 스캔, 트리맵, 대용량/오래된/중복 파일 인사이트, 최근 급성장 폴더, 빠른 정리, 파일 삭제 |
| **Docker** | 컨테이너 / 이미지 / 볼륨 / 빌드 캐시 관리 |
| **Cleanup** | 규칙 기반 정리, 미리보기, 검토 인박스, 자동화 일정/이력 |
| **Activity** | 프로세스, 포트, 포트 감시 |
| **DevTools** | 개발 환경 헬스, 개발 서버, 툴체인 캐시 정리, 포트 충돌, 프로젝트 모니터, 워크스페이스 |
| **Applications** | 설치 앱, 관련 데이터 정리, 잔여 데이터 정리, Windows 전용 잔여 레지스트리 정리 |
| **Preferences** | 테마, 언어, 알림 임계치, 스냅샷 주기, 업데이트 확인, 데이터/로그 경로 |
...
```text
src/
  main/        Electron 메인 프로세스 — IPC 핸들러와 서비스를 도메인 패키지로 그룹화(각 폴더에 index.ts 배럴)
  preload/     contextBridge 기반 renderer API
  renderer/    React UI — 페이지, 기능 모듈, 도메인별 스토어, ui/layout 컴포넌트
